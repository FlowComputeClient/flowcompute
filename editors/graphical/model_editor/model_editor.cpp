...
ModelEditor::ModelEditor(std::shared_ptr<RenderData> renderData, const QString& fullPath, int targetId,
                         QVulkanInstance* instance, bool isBinary, QWidget* parent):
    QWidget(parent), m_renderData(renderData), m_fullPath(fullPath), m_targetId(targetId),
    ...
    connect(m_leftPane, &ModelLeftPane::surfaceScaleRequested,
            this, &ModelEditor::onSurfaceScaleRequest);
    connect(m_leftPane, &ModelLeftPane::dirtyStateChanged,
            this, &ModelEditor::dirtyStateChanged);
}

// Update surface data
void ModelEditor::updateModel(std::shared_ptr<RenderData> newData) {
    ...
    std::array<float, 3> bounds;
    bounds[0] = m_renderData->boundingBoxMax[0] - m_renderData->boundingBoxMin[0];
    bounds[1] = m_renderData->boundingBoxMax[1] - m_renderData->boundingBoxMin[1];
    bounds[2] = m_renderData->boundingBoxMax[2] - m_renderData->boundingBoxMin[2];
    m_leftPane->setBounds(bounds);
...
void ModelEditor::changeBounds(double scaleFactor) {
...
void ModelEditor::onSurfaceScaleRequest(double scaleFactor) {
