...
// Helper to determine if a file is ASCII or Binary from its header
FoamFormat getFoamFormat(const fs::path& path) {
    std::ifstream file(path);
    std::string line;
    while (std::getline(file, line)) {
        if (line.find("format") != std::string::npos) {
            if (line.find("ascii") != std::string::npos) return FoamFormat::Ascii;
            if (line.find("binary") != std::string::npos) return FoamFormat::Binary;
        }
        if (line.find("// * * * *") != std::string::npos) break; // End of header
    }
...
std::vector<RenderPatch> parseBoundary(const fs::path& boundaryPath) {
    ...
    std::string word;
    // Skip header until the first '(' that opens the main list
    while (ss >> word && word != "(") {}

    RenderPatch currentPatch;
    bool inPatch = false;

    // 2. Parse the properly tokenized stringstream
    while (ss >> word) {
        // Break only if we hit the final closing parenthesis of the main list
        ...
        if (word == "{") {
            ...
            std::string semi; 
            ss >> semi; // Consumes the ';' which is now its own token
        } else if (inPatch && word == "startFace") {
            ss >> currentPatch.firstIndex;
            std::string semi; 
            ss >> semi; // Consumes the ';'
        } else if (!inPatch && !std::isdigit(static_cast<unsigned char>(word[0]))) {
            // Read patch name
            std::strncpy(currentPatch.name, word.c_str(), sizeof(currentPatch.name) - 1);
...
std::vector<double> readFoamPoints(const fs::path& path) {
    ...
    if (format == FoamFormat::Binary) {
        ...
        if (skipToBinary(file, numPoints)) {
            ...
            file.read(reinterpret_cast<char*>(rawPoints.data()), numPoints * 3 * sizeof(double));
        }
        ...
        if (numPoints > 0) {
            ...
            char c;
            while (file >> c && c != '(') {} // Skip to start of points list
...
FaceList readFoamFaces(const fs::path& path) {
    ...
    if (format == FoamFormat::Binary) {
        ...
        if (skipToBinary(file, totalOffsets)) {
            ...
            file.read(reinterpret_cast<char*>(result.offsets.data()), totalOffsets * sizeof(int32_t));
        }
        size_t totalIndices = 0;
        if (skipToBinary(file, totalIndices)) {
            result.connectivity.resize(totalIndices);
            file.read(reinterpret_cast<char*>(result.connectivity.data()), totalIndices * sizeof(int32_t));
        }
    } else if (format == FoamFormat::Ascii) {
        std::ifstream file(path);
        std::string line;
...
void triangulatePolygon(const std::vector<uint32_t>& poly,
    ...
    // This perfectly preserves the original CCW/CW winding order
...
RenderData read_mesh(const fs::path& casePath) {
    ...
    for (auto& patch : renderData.patches) {		
        ...
        patch.firstLineIndex = static_cast<uint32_t>(renderData.lineIndices.size());
        ...
        for (uint32_t f = foamStartFace; f < foamStartFace + foamNFaces; ++f) {
            ...
            if (f + 1 >= faces.offsets.size()) {
                std::cerr << "Error: Face index " << f << " out of bounds.\n";
                break; 
            }
            ...
            for (int32_t idx = startOffset; idx < endOffset; ++idx) {
                uint32_t globalPtIdx = static_cast<uint32_t>(faces.connectivity[idx]);

                // Ensure we don't read past the end of rawPoints or globalToLocal
                if (globalPtIdx >= totalRawPoints) {
                    std::cerr << "Error: Global point index " << globalPtIdx << " out of bounds.\n";
                    continue;
                }
                ...
                if (globalToLocal[globalPtIdx] == UINT32_MAX) {
                    ...
                    localPtIdx = static_cast<uint32_t>(renderData.data.size() / 3);
                    globalToLocal[globalPtIdx] = localPtIdx;

                    float x = static_cast<float>(rawPoints[globalPtIdx * 3]);
                    float y = static_cast<float>(rawPoints[globalPtIdx * 3 + 1]);
                    float z = static_cast<float>(rawPoints[globalPtIdx * 3 + 2]);
            ...
            for (size_t i = 0; i < numVerts; ++i) {
                ...
                renderData.lineIndices.push_back(localFaceLoop[(i + 1) % numVerts]);
            }

            triangulatePolygon(std::move(localFaceLoop), renderData.data, renderData.indices);
        }

        patch.indexCount =
            static_cast<uint32_t>(renderData.indices.size()) - patch.firstIndex;
        patch.lineIndexCount =
            static_cast<uint32_t>(renderData.lineIndices.size()) - patch.firstLineIndex;
    }

    renderData.boundingBoxMin = {minX, minY, minZ};
    renderData.boundingBoxMax = {maxX, maxY, maxZ};

	const uint32_t numLocalPoints = static_cast<uint32_t>(renderData.data.size() / 3);
    ...
    // Create a working copy of the offsets to track our insertion positions
    std::vector<uint32_t> currentInsertPos = renderData.pointOffsets;

    // Pre-allocate the adjacency array and scatter the data directly into place
