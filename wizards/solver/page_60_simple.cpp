...
SimplePage::SimplePage(QWidget *parent): QWizardPage(parent) {
    ...
    m_resTable->setHorizontalHeaderLabels(
        {"Enabled", "Field", "Residual Control" });
...
void SimplePage::initializePage() {
    ...
    for (int i = 0; i < cfg.resControls.size(); ++i) {
        ...
        m_resTable->setCellWidget(i, 0, centerCheck(enableCheck));

        // Column 1: Field Name
        QTableWidgetItem* fieldItem =
            new QTableWidgetItem(cfg.resControls[i].fieldName);
        ...
        QLineEdit* toleranceEdit =
            new QLineEdit(cfg.resControls[i].tolerance, this);
    ...
    m_resTable->setMaximumHeight(tableHeight);
}

bool SimplePage::validatePage() {
