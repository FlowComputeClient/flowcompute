...
#include "../../dialogs/surface/surface_dialog.h"
...
bool VisualizationPage::validatePage() {
    ...
    cfg->surfaceFormat = static_cast<Solver::SurfaceFormat>(
        m_surfaceFormatCombo->currentData().toInt());
    cfg->writeControl = static_cast<Solver::VisWriteControlType>(
        m_writeControlCombo->currentData().toInt());
    cfg->writeInterval = m_writeIntervalEdit->text().toDouble();
    cfg->interpolationType = static_cast<Solver::InterpolationType>(
