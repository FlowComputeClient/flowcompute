...
ParallelPage::ParallelPage(QWidget *parent): QWizardPage(parent) {
    ...
    QGroupBox* parallelGroup =
        new QGroupBox(tr("Decomposition Settings"), this);
    ...
    parallelLayout->addRow(tr("Employ parallel decomposition:"),
                           m_parallelCheck);
    ...
    parallelLayout->addRow(tr("Set the number of subdomains:"),
                           m_subdomainsSpin);
    ...
    connect(m_methodCombo, &QComboBox::currentIndexChanged, this,
            &ParallelPage::methodChanged);
    ...
    hierarchicalLayout->addRow(tr("Subdomains in the x-direction: "),
                               m_hierXSpin);

    // Subdomains in the Y-direction
    m_hierYSpin = new QSpinBox();
    m_hierYSpin->setMinimum(1);
    hierarchicalLayout->addRow(tr("Subdomains in the y-direction: "),
                               m_hierYSpin);

    // Subdomains in the Z-direction
    m_hierZSpin = new QSpinBox();
    m_hierZSpin->setMinimum(1);
    hierarchicalLayout->addRow(tr("Subdomains in the z-direction: "),
                               m_hierZSpin);
    ...
    hierarchicalLayout->addRow(tr("Cell skew factor (delta): "),
                               m_hierDeltaSpin);
...
void ParallelPage::initializePage() {
    ...
    int methodIndex =
        m_methodCombo->findData(QVariant::fromValue(m_cfg->method));
    if (methodIndex != -1) {
    ...
    if (m_cfg->method == FlowCompute::DecompositionMethod::Simple ||
        ...
        if ((nx * ny * nz) != m_cfg->numSubdomains) {
            QMessageBox::warning(this, tr("Invalid Geometry"),
                tr("The product of x, y, and z must equal "
                    "the number of subdomains."));

            // Snap focus back to the input
            activeX->setFocus();
            activeX->selectAll();
            return false;
        }
