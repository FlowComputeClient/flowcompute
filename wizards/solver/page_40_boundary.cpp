...
BoundaryPage::BoundaryPage(
    ...
    // Event-handling
    connect(m_internalFieldEdit, &QLineEdit::textEdited, this,
    ...
    for (auto fieldIt = m_cfg->begin(); fieldIt != m_cfg->end(); ++fieldIt) {
        ...
        if (fieldData.internalField.trimmed().isEmpty()) {
            QMessageBox::warning(this, tr("Validation Error"),
                tr("The internal field for '%1' cannot be empty.")
                                     .arg(fieldName));
            return false;
        }
        ...
        for (const auto& bcPair : fieldData.bcs) {
            ...
            if (bc.type.trimmed().isEmpty()) {
                QMessageBox::warning(this, tr("Validation Error"),
                     tr("No boundary condition type selected for "
                        "patch '%1' in field '%2'.").arg(patchName, fieldName));
                return false;
            }
...
void BoundaryPage::onPatchSelected(const QString& patch) {
    ...
    if (patch == "< Internal >") {
        ...
        for (const auto& bc : std::as_const(m_boundaryConditions)) {
            if (bc.types.contains(fieldClassName) &&
                bc.patchTypes.contains(patchType)) {
...
void BoundaryPage::onBcTypeChanged(const QString& bcType) {
    ...
    for (const auto& param : std::as_const(params)) {
        ...
        // Update state data
        connect(paramEdit, &QLineEdit::textEdited, this,
                [this, param](const QString& text) {
