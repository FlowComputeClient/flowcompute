...
PhysicsPage::PhysicsPage(const std::vector<FlowCompute::SolverFamily>& families,
    ...
    QGroupBox* turbulenceGroup = new QGroupBox(tr("Turbulence Selection"), this);
    layout->addWidget(turbulenceGroup);
    QFormLayout* turbulenceLayout = new QFormLayout(turbulenceGroup);

    // Create the label
    turbulenceLayout->addRow(new QLabel(tr("<b>Select the turbulence model for the simulation:</b>")));
    ...
    turbulenceLayout->addRow(tr("LES filter width (Delta):"), m_deltaModelCombo);
    ...
    m_propertiesTable->setHorizontalHeaderLabels({ "Property", "Variable", "Dimension", "Value" });
    ...
    m_propertiesTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Stretch);
    m_propertiesTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Stretch);
    m_propertiesTable->setSelectionMode(QAbstractItemView::NoSelection);
    transportLayout->addRow(m_propertiesTable);

    transportLayout->addItem(new QSpacerItem(0, 0, QSizePolicy::Minimum, QSizePolicy::Expanding));
}
...
void PhysicsPage::initializePage() {
    ...
    // Update combo boxes
    m_transportModelCombo->setCurrentIndex(static_cast<int>(m_cfg->transportModel));
    m_deltaModelCombo->setCurrentIndex(static_cast<int>(m_cfg->deltaModel));
    ...
    int height = m_propertiesTable->horizontalHeader()->height() + m_propertiesTable->frameWidth() * 2;
...
bool PhysicsPage::validatePage() {
    ...
    if (selectedItems.isEmpty()) {
        QMessageBox::warning(this, tr("Selection Required"), tr("Please select a turbulence model."));
        return false;
    }
    ...
    m_cfg->transportModel = static_cast<Solver::TransportModel>(m_transportModelCombo->currentIndex());
    m_cfg->deltaModel = static_cast<Solver::DeltaModel>(m_deltaModelCombo->currentIndex());
